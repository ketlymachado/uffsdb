...
/* Estrutura global que guarda as informações obtidas pelo yacc
 * na identificação dos tokens para o SELECT
 */
rc_select GLOBAL_DATA_SLCT;
...
void setObjName(char **nome) {
	if (GLOBAL_PARSER.mode == OP_SELECT) {
		GLOBAL_DATA_SLCT.objName = malloc(sizeof(char)*((strlen(*nome)+1)));

        strcpylower(GLOBAL_DATA_SLCT.objName, *nome);
        GLOBAL_DATA_SLCT.objName[strlen(*nome)] = '\0';
        GLOBAL_PARSER.step++;
    } else if (GLOBAL_PARSER.mode != 0) {
...
void setColumnSelect(char **nome) {
    GLOBAL_DATA_SLCT.columnName = realloc(GLOBAL_DATA_SLCT.columnName, (GLOBAL_PARSER.col_count+1)*sizeof(char *));

    GLOBAL_DATA_SLCT.columnName[GLOBAL_PARSER.col_count] = malloc(sizeof(char)*(strlen(*nome)+1));
    strcpylower(GLOBAL_DATA_SLCT.columnName[GLOBAL_PARSER.col_count], *nome);
    GLOBAL_DATA_SLCT.columnName[GLOBAL_PARSER.col_count][strlen(*nome)] = '\0';
...
void clearGlobalStructs() {
	...
	if (GLOBAL_DATA_SLCT.objName) {
        free(GLOBAL_DATA_SLCT.objName);
        GLOBAL_DATA_SLCT.objName = NULL;
    }

    for (i = 0; i < GLOBAL_DATA_SLCT.N; i++ ) {
        if (GLOBAL_DATA_SLCT.columnName)
